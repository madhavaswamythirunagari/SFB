...
public class results2 extends HttpServlet
{
              ...
              throws ServletException, IOException
    {
		...
		int totnumst=0;
			...
			try
			{
				...
				String Sq="SELECT FNAME,SUBJECT, avg(g1) AS gd1,avg(g2) AS gd2,avg(g3) AS gd3, avg(g4) AS gd4, avg(g5) AS gd5, avg(g6) AS gd6, avg(g7) AS gd7, avg(g8) AS gd8, avg(g9) AS gd9, avg(g10) AS gd10, count(*) AS numst ,avg([TOT-PER]) AS per FROM tot_stud where class='"+t_class+"'  and year='"+t_year+"' GROUP BY fname, subject";
				...
				// rs  = stmt.executeQuery(Sq);
					 ...
					 String grades[]={"Passion and Enthusiasm  to Teach","Subject Knowledge", 
										"Clarity and Emphasis on Concepts", 
										"Motivating and Inspiring the Student", 
										"Creating Interest in the Subject", 
										"Quality of Illustrative Visuals, Examples and Applications", 
										"Regularity, Punctuality and Uniform Coverage of Syllabus", 
										"Discipline and Control over the Class", 
										"Promoting Student Thinking", 
										"Encouraging Student Effort and Inviting Student Interaction"}; 
		...
		out.println("<html><body>");
			  ...
			  out.println("<BODY bgcolor=navyblue>");
					...
					out.println("<font size=6 color=yellow>Student FeedBack Form</font>");
					out.println("</H1>");
					out.println("<hr />");
...
//---------------------------------------------------------------------
					...
					while(r<c)
					{
						...
						while(i<1)
						{
							tt=tt+n[r][0]*(float)0.15+n[r][1]*(float)0.15+n[r][2]*(float)0.2+n[r][3]*(float)0.1+n[r][4]*(float)0.05+n[r][5]*(float)0.05+n[r][6]*(float)0.1+n[r][7]*(float)0.1+n[r][8]*(float)0.05+n[r][9]*(float)(0.05);
							//tt=tt+n[r][0]*(float)0.15;
							i=i+1;
						}
						//totp=tot/10;
						out.println("<td bgcolor=#00BFFF><b>"+tt+"</b></td>");
						r=r+1;
					}
					out.println("</tr>");
